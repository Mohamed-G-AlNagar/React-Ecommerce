...
const ProductDetails = () => {
  //? used to receive data sent to this page when navigate to it
  //? as ==> navigate(`/product/${item._id}`, { state: { item } });

  const location = useLocation();
  const { item: product } = location.state;
  console.log(product, 'productSent');
  const { id } = useParams<{ id?: string }>();
  const { mutate: addItemToCart } = useAddItemToCart();

  //? useQuery Way but instead i used useLocation to get sended data
